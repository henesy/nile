...
usage(void)
{
	fprint(2, "usage: nile [-f font] [-i initcmd] [-k kbdcmd] [-s]\n");
	exits("usage");
}
...
threadmain(int argc, char *argv[])
{
	...
	if(access(fontname, 0) < 0){
		fprint(2, "nile: can't access %s: %r\n", fontname);
		exits("font open");
	}
	...
	if(geninitdraw(nil, derror, nil, "nile", nil, Refnone) < 0){
		fprint(2, "nile: can't open display: %r\n");
		exits("display open");
	}
	iconinit();
	view = screen;
	...
	if(filsys == nil)
		fprint(2, "nile: can't create file system server: %r\n");
	else{
				...
				if(w==nil || (w==winput && w->topped>0)){
					if(mouse->buttons & 1){
						...
						if(winput && !winput->mouseopen)
							if(menuless) {
								/* Factor out w/ button2menu version? */
								getsnarf();
								wsnarf(winput);
								if(nsnarf == 0)
									break;
								if(winput->rawing){
									waddraw(winput, snarf, nsnarf);
									if(snarf[nsnarf-1]!='\n' && snarf[nsnarf-1]!='\004')
										waddraw(winput, L"\n", 1);
								}else{
									winsert(winput, snarf, nsnarf, winput->nr);
									if(snarf[nsnarf-1]!='\n' && snarf[nsnarf-1]!='\004')
										winsert(winput, L"\n", 1, winput->nr);
								}
								wsetselect(winput, winput->nr, winput->nr);
								wshow(winput, winput->nr);
//								goto Drain;
								wsendctlmesg(winput, Wakeup, ZR, nil);
								flushimage(display, 1);
							}else
								button2menu(winput);
						goto Drain;
...
{
	sweeping = 1;
	switch(menuhit(3, mousectl, &menu3, wscreen)){
	case -1:
		break;
	case Delete:
		delete();
...
button1menu(void)
{
	...
	for(i=0; i<nhidden; i++)
		menu1str[i] = hidden[i]->label;
	menu1str[i] = nil;

	sweeping = 1;
	switch(i = menuhit(1, mousectl, &menu1, wscreen)){
	case -1:
		break;
	default:
		unhide(i);
...
sweep(void)
{
	...
	drawgetrect(or, 0);
	i = allocwindow(wscreen, or, Refnone, 0xEEEEEEFF);
	if(i == nil)
		goto Rescue;
